  ...
  switch (type) {
    ...
    case ADD_PRODUCT: {
      try {
        ...
        const keyFn = yield call(firebase.generateKey);
        const key = typeof keyFn === "function" ? keyFn() : keyFn;

        // ✅ NO-STORAGE fallback: koristimo URL umesto upload-a
        const PLACEHOLDER = (seed) =>
          `https://picsum.photos/seed/${seed}/800/800`;

        // Ako payload.image već jeste string URL, koristi ga.
        // Ako je File (upload), preskoči i koristi placeholder.
        const imageUrl =
          typeof payload.image === "string" && payload.image.trim() !== ""
            ? payload.image
            : PLACEHOLDER(key);

        // Image collection: zadržavamo sve što je već URL, a file-ove mapiramo na placeholder
        const images =
        ...
        const product = {
          ...
          // čuvamo URL (ne File)
          image: imageUrl,
          // osiguraj da je array objekata {id, url}
